...
__declspec(dllimport) gvplugin_library_t gvplugin_gd_LTX_library;
__declspec(dllimport) gvplugin_library_t gvplugin_gdiplus_LTX_library;
__declspec(dllimport) gvplugin_library_t gvplugin_neato_layout_LTX_library;
__declspec(dllimport) gvplugin_library_t gvplugin_pango_LTX_library;
__declspec(dllimport) gvplugin_library_t gvplugin_rsvg_LTX_library;
__declspec(dllimport) gvplugin_library_t gvplugin_webp_LTX_library;
}
...
ImGraphvizContext * CreateContext()
{
    IM_ASSERT(g_ctx.gvcontext == nullptr);
    g_ctx.gvcontext = gvContext();

    gvAddLibrary(g_ctx.gvcontext, &gvplugin_core_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_dot_layout_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_gd_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_gdiplus_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_neato_layout_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_pango_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_rsvg_LTX_library);
    gvAddLibrary(g_ctx.gvcontext, &gvplugin_webp_LTX_library);
...
bool BeginNodeGraph(char const * id, ImGraphvizLayout layout, float pixel_per_unit)
{
...
void NodeGraphAddNode(char const * id)
{
...
void NodeGraphAddNode(char const * id, ImVec4 const & color, ImVec4 const & fillcolor)
{
    auto & cache = g_ctx.graph_caches[g_ctx.lastid];
    IM_ASSERT(g_ctx.gvgraph != nullptr);
    Agnode_t * const n = agnode(g_ctx.gvgraph, ImGuiIDToString(id), 1);
    IM_ASSERT(n != nullptr);
    IMGRAPHVIZ_CREATE_LABEL_ALLOCA(text, id);
...
void NodeGraphAddEdge(char const * id, char const * node_id_a, char const * node_id_b)
{
    ImVec4 const color = ImGui::GetStyle().Colors[ImGuiCol_Text];
    NodeGraphAddEdge(id, node_id_a, node_id_b, color);
}

void NodeGraphAddEdge(char const * id, char const * node_id_a, char const * node_id_b, ImVec4 const & color)
{
    auto & cache = g_ctx.graph_caches[g_ctx.lastid];
    IM_ASSERT(g_ctx.gvgraph != nullptr);
...
int ImGraphvizFillDrawNodeBuffer(
    ImGraphviz_Graph const & graph, ImGraphviz_DrawNode * drawnodes, ImVec2 cursor_pos, float ppu)
{
    int const count = (int)graph.nodes.size();

    if (drawnodes)
    {
        constexpr int num_segments = IMGRAPHVIZ_DRAW_NODE_PATH_COUNT - 1;
        ...
        {
            ImGraphviz_Node const & node = graph.nodes[i];
            ImVec2 const textsize = ImGui::CalcTextSize(node.label.c_str());

            for (int j = 0; j <= num_segments; j++)
            {
                float const a = a_min + ((float)j / (float)num_segments) * (a_max - a_min);
                drawnodes[i].path[j].x = cursor_pos.x + (node.pos.x + ImCos(a) * node.size.x / 2.f) * ppu;
                drawnodes[i].path[j].y
                    = cursor_pos.y + ((graph.size.y - node.pos.y) + ImSin(a) * node.size.y / 2.f) * ppu;
            }
...
int ImGraphvizFillDrawEdgeBuffer(
    ImGraphviz_Graph const & graph, ImGraphviz_DrawEdge * drawedges, ImVec2 cursor_pos, float ppu)
{
    int const count = (int)graph.edges.size();

    if (drawedges)
    {
        constexpr int points_count = IMGRAPHVIZ_DRAW_EDGE_PATH_COUNT;
        static_assert(points_count > 1, "");

        for (int i = 0; i < count; ++i)
        {
            ImGraphviz_Edge const & edge = graph.edges[i];
            ImVec2 const textsize = ImGui::CalcTextSize(edge.label.c_str());

            for (int x = 0; x < points_count; ++x)
            {
                drawedges[i].path[x]
                    = ImGraphviz_BezierVec2(edge.points.data(), (int)edge.points.size(), x / float(points_count - 1));
...
void EndNodeGraph()
{
    ...
    {
        ImGraphvizRenderGraphLayout(cache.graph, cache.layout);
    ...
    {
        cache.drawnodes.resize(ImGraphvizFillDrawNodeBuffer(cache.graph, nullptr, cursor_pos, ppu));
        ImGraphvizFillDrawNodeBuffer(cache.graph, cache.drawnodes.data(), cursor_pos, ppu);
        cache.drawedges.resize(ImGraphvizFillDrawEdgeBuffer(cache.graph, nullptr, cursor_pos, ppu));
        ImGraphvizFillDrawEdgeBuffer(cache.graph, cache.drawedges.data(), cursor_pos, ppu);
        cache.cursor_previous = cache.cursor_current;
    }

    for (auto const & node : cache.drawnodes)
    {
        drawlist->AddConvexPolyFilled(node.path, IMGRAPHVIZ_DRAW_NODE_PATH_COUNT, node.fillcolor);
        drawlist->AddPolyline(node.path, IMGRAPHVIZ_DRAW_NODE_PATH_COUNT, node.color, ImDrawFlags_Closed, 1.f);
    ...
    for (auto const & edge : cache.drawedges)
    {
        ...
        drawlist->AddPolyline(edge.path, IMGRAPHVIZ_DRAW_EDGE_PATH_COUNT, edge.color, ImDrawFlags_None, 1.f);
